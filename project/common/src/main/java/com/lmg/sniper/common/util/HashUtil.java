...
public class HashUtil {
    ...
    public HashUtil(String salt, int minHashLength) {
        this(salt, minHashLength, DEFAULT_ALPHABET);
    }

    public HashUtil(String salt, int minHashLength, String alphabet) {
        ...
        seps = HashUtil.consistentShuffle(seps, this.salt);
        ...
        alphabet = HashUtil.consistentShuffle(alphabet, this.salt);
  ...
  /* Private methods */
    ...
    private String _encode(long... numbers) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            ...
            alphabet = HashUtil.consistentShuffle(alphabet, buffer.substring(0, alphabet.length()));
            String last = HashUtil.hash(num, alphabet);
        ...
        while (ret_str.length() < this.minHashLength) {
            alphabet = HashUtil.consistentShuffle(alphabet, alphabet);
    ...
    private long[] _decode(String hash, String alphabet) {
        ...
        if (!hashBreakdown.isEmpty()) {
            ...
            for (String aHashArray : hashArray) {
                ...
                alphabet = HashUtil.consistentShuffle(alphabet, buffer.substring(0, alphabet.length()));
                ret.add(HashUtil.unhash(subHash, alphabet));
            }
        }
    ...
    public static void main(String[] args) {
        HashUtil hash = new HashUtil("hmbst", 8);
        System.out.println(new HashUtil("password", 8, "0123456789").encode(new Random().nextInt(10000)));
    }
}